...
//
// Why this exists (mirrors workbuddy/counter.go): the panel's 成功/失败
// columns historically came from the host's recent-request window
// (HostAuthFileEntry.Success / Failed) — memory-only rolling counters that
// zero out on container restart. This module replaces that with counters the
// plugin itself owns and persists into the physical auth file's top-level
...
//     account UID (traeAuth.UserID — the executor's stable account identity,
//     the same key the scheduler / failover / anomaly layers use).
...
// would create an ambiguous dual source of truth.
//
// UID-less accounts are skipped (no stable key to map a counter back to a
// physical file on flush) — they fall back to the host's recent-window
// numbers in the panel (unchanged behavior).
package main
...
// loadCountersFromDisk walks every traework auth file and seeds the in-memory
...
func loadCountersFromDisk() {
	...
	for _, f := range files {
		...
		uid := strings.TrimSpace(sa.UserID)
		if uid == "" {
			continue
		}
