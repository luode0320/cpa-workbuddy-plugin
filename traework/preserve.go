...
//
// Preserve is a *runtime health gate* (mirrors workbuddy/preserve.go): the
// watchdog (watchdog.go) refreshes credits every interval (default 10m) and
// shields any account whose balance dropped below the threshold, then releases
// it automatically when credits recover. Accounts not in the preserve set are
// "normal" — they route normally.
//
// Routing contract (scheduler.pick):
//  1. Drop disabled accounts.
//  2. Drop preserve accounts (this file).
//  3. Drop anomaly accounts (anomaly.go).
//  4. Drop cooling-down accounts (accountFailover.go).
//  5. Route on the survivors (all "normal" accounts are equal).
//
// Preserve filter is *before* anomaly/cooldown so a freshly preserved account
// that then 429s doesn't double-count. When EVERY traework account is
// preserved the full list is kept so the pickers fall back to the current pin
// (fleet-wide credit reset must not lock routing).
package main
...
// plugin config_yaml in configure(); readers take the RW lock so a concurrent
// reconfigure is safe.
var (
...
// top-level fields the host doesn't recognize (same root cause as anomaly /
// manual_disable).
...
// preserved alongside disabled / note / anomaly / etc.
//
...
func persistPreserveToggle(authIndex, authID string, on bool) error {
	...
	if err := json.Unmarshal(phys.JSON, &doc); err != nil {
		...
		// direct-write toggles such as anomaly).
		doc = map[string]any{}
	}
	if on {
		doc["preserve"] = true
