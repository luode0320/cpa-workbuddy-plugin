...
func TestCollectTraeStreamReportsToolCalls(t *testing.T) {
	chunks, hasToolCalls, err := collectTraeStream(strings.NewReader(toolCallSSE), "qwen3.8-max", 200)
	if err != nil {
...
`
	_, hasToolCalls, err := collectTraeStream(strings.NewReader(sse), "qwen3.8-max", 200)
	if err != nil {
