...
plugins:
  configs:
    workbuddy:
      ...
      # 同请求内账号级切号预算（默认 10，范围 0-10）。
      # v0.14.0 起 cover 账号级 4xx（401/403/404/405）—— token 过期、
      # 无权限、endpoint 缺失、方法不允许——以及 v0.14.2 起增加 429
      # 软限流（假设上游按账号/租户切分限流，切下一个候选账号即可缓解）。
      # 命中后插件会把同一次请求重放到下一个可用账号，直到超过
      # retry_on_4xx 次为止；失败的账号同时进入固定 15s cooldown，下次
      # 请求自动跳过。
      # 5xx/0/402 仍走 cooldown 跨请求路径（不进本次切号循环）；
      # 400 永不重试（请求级错误，换账号也是同样结果）。
      # 0 关闭同请求切号（异常期 kill switch）。
      retry_on_4xx: 10
