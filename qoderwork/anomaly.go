// anomaly.go manages the "anomaly" status for qoderwork-provider — accounts
...
//
// Anomaly is a *lifetime health gate*: when a qoderwork account has hit
...
//     writeAuthFileDirect). qoderwork does NOT have a per-plugin
//     writeAuthFileDirect yet (authfile.go only goes through host.auth.save),
//     so we inline the atomic temp+rename helper here and confine it to
//     safe qoderwork auth paths.
//
...
// writeAnomalyFileDirect is a qoderwork-local copy of workbuddy's
// writeAuthFileDirect. Inlined here because qoderwork's authfile.go
// intentionally routes every credential write through host.auth.save
// (preserves the no-direct-write architecture). We still need a
// direct-write path for the anomaly top-level flag because the host may
// drop unrecognized fields when it rebuilds the auth record.
//
...
func writeAnomalyFileDirect(path string, raw []byte) error {
	...
	tmp, err := os.CreateTemp(dir, ".qoderwork-anomaly-*.tmp")
	if err != nil {
...
// handleUnfreezeAuth is the qoderwork management endpoint backing the
