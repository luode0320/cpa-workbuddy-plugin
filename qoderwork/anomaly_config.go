...
//
// Mirrors workbuddy/anomaly_config.go so the two plugins stay in lockstep
// on the cooldown-vs-anomaly split: qoderwork has the older retry-only
// failover model (no preserve/watchdog), so anomaly is its first
// lifetime-health gate. Defaults are intentionally identical so config
// recipes are portable across both plugins.
package main
