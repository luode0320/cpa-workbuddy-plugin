...
func pumpUpstreamStream(httpReq *http.Request, cancel context.CancelFunc, streamID string, sseFramed bool, requestedModel, upstreamModel, authUID string, started time.Time, authID, accountLabel, sessionKey string) {
	...
	// 前置冷却拦截：若初始账号在冷却中，直接切换到健康账号，不发送自杀式请求
	if encodedBody != "" && (isAccountCoolingDown(curAuthID) || isAccountCoolingDown(curAuthUID)) {
		if nextID, nextSA, hasNext := pickNextAuth(curAuthID); hasNext && nextSA != nil {
			if nextReq, rebErr := rebuildRequestWithQoderAuth(nextSA, encodedBody, upstreamModel); rebErr == nil {
				curReq = nextReq
				curAuthID = nextID
	...
	for attempt := 0; attempt <= budget; attempt++ {
		...
		if sseErr != "" {
			// SSE error frame on HTTP 200: the current account must enter
			// failover cooldown. Remap to 403 so isAccountFailure classifies
			// it as account-level and the account gets its 15s cooldown.
