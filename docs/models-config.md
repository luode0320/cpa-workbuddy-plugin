...
1. **`models:` 值支持单行与多行 JSON（2026-08-28 起）**：冒号后可以跟完整 JSON 数组（单行，推荐），也可以是面板/编辑器自动格式化出的**多行 pretty-print**（`models: [` 换行、逐对象一行的形态）——插件已按括号配对跨行收集后整体解析。多行 YAML 列表（`models:` 换行逐行 `- xxx`）**仍不解析**，保持旧值。
