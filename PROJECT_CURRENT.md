...
- 状态：活跃维护中。生产现役与 registry 对齐：workbuddy-provider **0.14.18** / qoderwork-provider **0.9.6** / traework-provider **0.1.32** / workbuddy-token-usage **0.2.2**。traework 0.1.32 已发布并热加载部署生效（active_version=0.1.32 retired=0.1.31，落盘 .so sha256 4a4546d9...e694 与本地 zip 一致）。0.1.30 修复伪完成三缺陷（async 401 核算驱逐 + 单号池同号退避 + reasoning 纳入伪完成判据）；0.1.31 修复思考型长推理 reasoning 阶段零字节 504（pump gate 双轴健康度流式放行，stream 2999 生产复验 490.7s 完整返回 8702+55062 字符）；0.1.32 修复工具调用链路（P1 hasToolCalls 贯通伪完成豁免工具短流 + P0 tools 上行透传/parameters stringify/function↔function_call 双向翻译/三路径 tool_calls delta/finish=tool_calls）。workbuddy-token-usage **0.2.2**（2026-09-03）修复 dashboard 实时性——feed 新增 usage 经 SSE 通知（feedNotifier seq + /usage/events + EventSource 短连接轮询，15s 轮询 fallback）。
...
- 当前首要执行点：**traework 0.1.32 已发布部署生效，行为级验收待用户真实工具链流量**（见「已完成」0.1.32 条目与活动会话首行）。P1/P0 已闭环：伪完成豁免工具短流 + 完整工具链转换（tools 上行/历史键翻译/下行 delta）。**待观察**：用户真实 CodeBuddy 工具调用请求（stream#3206/3208「回答不完整」不再复现、工具轮完整 done + 后续续答正常；深度思考类长请求不 504 的 0.1.31 成果不回退）。
